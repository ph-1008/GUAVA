#!/usr/bin/env python3
"""
完整的追踪数据串接工具（使用 motion envelope + 強化參數平滑）
增強版本：添加平滑效果驗證
"""
...
def velocity_regions(active_indices):
...
def expand_regions_to_low_threshold(regions, velocity, low_threshold):
    """Expand active regions until motion returns below the lower threshold."""
    expanded = []
    last_idx = len(velocity) - 1
    for start, end in regions:
        while start > 0 and velocity[start - 1] > low_threshold:
            start -= 1
        while end < last_idx and velocity[end + 1] > low_threshold:
            end += 1
        expanded.append([start, end])
    return expanded

def merge_close_regions(regions, max_gap):
    """Merge neighboring motion regions separated by a short pause."""
    if not regions:
        return []

    regions = sorted(regions, key=lambda item: item[0])
    merged = [regions[0]]
...
def enforce_min_segment_length(start_frame, end_frame, num_frames, min_segment_frames):
    """Expand a short segment around its center."""
    current_len = end_frame - start_frame
    if current_len >= min_segment_frames:
        return start_frame, end_frame

    center = (start_frame + end_frame) // 2
    half = min_segment_frames // 2
    start_frame = max(0, center - half)
    end_frame = min(num_frames, start_frame + min_segment_frames)
    start_frame = max(0, end_frame - min_segment_frames)
    return start_frame, end_frame

def detect_sign_segment_by_peaks(tracking_data, window_size=5, min_peak_distance=10, offset_frames=3):
    """基於手部 motion envelope 的手語片段偵測，peaks 只用於輔助視覺化。"""
    ...
    if num_frames <= 1 or np.all(velocities == 0):
        print(f"    ⚠️  警告：所有速度都是 0，使用全部幀")
        return 0, num_frames, velocities, np.array([])

    smooth_sigma = max(1.0, window_size / 2.5)
    smoothed_velocity = gaussian_filter1d(velocities, sigma=smooth_sigma)
    ...
    velocity_mad = float(np.median(np.abs(smoothed_velocity - velocity_median)))
    ...
    if num_frames <= 25 or velocity_p90 - velocity_p35 < 1e-3:
        print(f"    短片段或速度變化不足，保守使用全部幀")
        peaks, _ = find_peaks(smoothed_velocity, distance=max(1, min_peak_distance // 2))
        return 0, num_frames, smoothed_velocity, peaks
    ...
    peak_height_threshold = high_threshold
    ...
    print(f"    Robust 閾值: low={low_threshold:.2f}, high={high_threshold:.2f}")
    print(f"    找到 {len(peaks)} 個輔助峰值")

    active_indices = np.where(smoothed_velocity > high_threshold)[0]
    regions = velocity_regions(active_indices)
    regions = expand_regions_to_low_threshold(regions, smoothed_velocity, low_threshold)

    if regions:
        max_peak = max(float(np.max(smoothed_velocity[start:end + 1])) for start, end in regions)
        max_energy = max(float(np.sum(smoothed_velocity[start:end + 1])) for start, end in regions)
        filtered_regions = []
        for start, end in regions:
            region_peak = float(np.max(smoothed_velocity[start:end + 1]))
            region_energy = float(np.sum(smoothed_velocity[start:end + 1]))
            region_len = end - start + 1
            if region_len >= 2 and (region_peak >= max_peak * 0.25 or region_energy >= max_energy * 0.15):
                filtered_regions.append([start, end])
        regions = filtered_regions or regions

    merge_gap = max(min_peak_distance, 10)
    regions = merge_close_regions(regions, merge_gap)

    if regions:
        start_velocity_idx = regions[0][0]
        end_velocity_idx = regions[-1][1]
        # pre_pad = max(offset_frames + 2, 5)
        # post_pad = max(offset_frames + 5, 8)
        pre_pad = offset_frames
        post_pad = offset_frames
        # start_frame = start_velocity_idx - pre_pad
        # end_frame = end_velocity_idx + 2 + post_pad
        start_frame = start_velocity_idx + pre_pad
        end_frame = end_velocity_idx - post_pad
        print(f"    動作區間: {regions}")
        # print(f"    加 padding: start={start_velocity_idx}-{pre_pad}, end={end_velocity_idx}+2+{post_pad}")
        print(f"    加 padding: start={start_velocity_idx}+{pre_pad}, end={end_velocity_idx}-{post_pad}")
    else:
        print(f"    未找到穩定動作區間，保守使用中間大段")
        start_frame = offset_frames
        end_frame = num_frames - offset_frames

    start_frame = max(0, start_frame)
    end_frame = min(num_frames, end_frame)

    min_segment_frames = min(num_frames, max(18, int(num_frames * 0.45)))
    old_start, old_end = start_frame, end_frame
    start_frame, end_frame = enforce_min_segment_length(
        start_frame, end_frame, num_frames, min_segment_frames
    )
    if (start_frame, end_frame) != (old_start, old_end):
        print(f"    片段過短，擴張至至少 {min_segment_frames} 幀: {start_frame} → {end_frame}")
...
def concatenate_tracking_data_with_peaks(tracked_dirs, output_dir, 
                                        ...
                                        use_median_for_hands=True,
                                        fix_hand_jitter=True,
                                        visualize=True):
    """使用 motion envelope 方法串接追踪数据（增強驗證版）"""
    print(f"\n{'='*70}")
    print(f"手語追蹤數據串接工具（Motion Envelope + 強化參數平滑 + 驗證）")
    print(f"{'='*70}")
    ...
    print(f"  ├─ 手部閃動修復: {'啟用' if fix_hand_jitter else '停用'}")
    ...
    # 載入所有追踪数据並使用 motion envelope 裁切
    for idx, tracked_dir in enumerate(tracked_dirs):
        ...
        if fix_hand_jitter:
            ...
            print("  跳過單詞片段 hand jitter 修復，保留原始 EHM-Tracker 手部參數")
        ...
        print(f"  使用 motion envelope 分析手語動作...")
        ...
        if is_first:
            start_frame = 0
            print(f"    第一個視頻：保留開頭（第 0 幀），結尾使用 motion envelope (幀 {end_frame})")
                
        if is_last:
            end_frame = len(frame_keys)
            print(f"    最後一個視頻：開頭使用 motion envelope (幀 {start_frame})，保留結尾（第 {end_frame} 幀）")
            
        if not is_first and not is_last:
            print(f"    中間視頻：使用 motion envelope 裁剪")
        ...
        if visualize:
            ...
            ax.set_title(f'Motion Envelope: {Path(tracked_dir).name}', fontsize=14, fontweight='bold')
    ...
    print(f"✓ 參數平滑: {'已應用（強化版 + 驗證）' if smooth_params else '停用'}")
    print(f"✓ 手部閃動修復: {'已應用' if fix_hand_jitter else '停用'}")
