...
def some_evaluation_function(model, val_dataset, tokenizer, device):
    rouge = Rouge()
    predictions = []
    references = []
...
for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
    ...
    output_dir = f"{base_output_dir}/results_fold_{fold}"
    
    training_args = TrainingArguments(
        output_dir=f"{base_output_dir}/fold_{fold}",
        eval_strategy="epoch",
        learning_rate=5e-5,
        warmup_steps=100,
        ...
        logging_dir=f"{base_output_dir}/logs_fold_{fold}",
    )
    ...
    model_score = some_evaluation_function(model, val_dataset, tokenizer, device)
    ...
    if model_score > best_score:
        ...
        best_val_df = val_df
        model.save_pretrained(f"{base_output_dir}")
        tokenizer.save_pretrained(f"{base_output_dir}")

        if best_train_df is not None and best_val_df is not None:
            best_train_df.to_csv(f"{base_output_dir}/best_train.csv", index=False)
            best_val_df.to_csv(f"{base_output_dir}/best_val.csv", index=False)

print(f"最佳的 fold 為 {best_fold}，已保存最佳模型於 {base_output_dir}。")

import shutil
for fold_dir in [f"{base_output_dir}/fold_{i}" for i in range(5)]:
    if fold_dir != f"{base_output_dir}/fold_{best_fold}" and fold_dir != f"{base_output_dir}/fold_{best_fold+1}" :
