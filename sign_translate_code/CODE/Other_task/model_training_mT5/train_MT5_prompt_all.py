...
dataset["input_text"] = dataset.apply(lambda row: f"translate Chinese to Gloss: {row['input_text']} <POS> {' '.join(row['pos'])} <DEP> {' '.join([i[1] for i in row['dep']])} <CON> {row['con']}", axis=1)


if __name__ == "__main__":
    # 取得當前時間
    current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M")
    base_output_dir = os.path.join(script_dir, f"{current_time}_mt5_prompt_all") # New version ID
