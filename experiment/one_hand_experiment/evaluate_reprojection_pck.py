...
def main() -> None:
    ...
    parser.add_argument(
        "--out-csv",
        type=Path,
        default=DEFAULT_ROOT / "tracking_step_1_2_hands_comparison_reprojection_pck.csv",
    )
    parser.add_argument(
        "--out-md",
        type=Path,
        default=DEFAULT_ROOT / "tracking_step_1_2_hands_comparison_reprojection_pck_report.md",
    )
    ...
    if not cases:
        raise SystemExit(f"No tracking cases found under {', '.join(str(root) for root in args.root)}")
...
def discover_cases(roots: list[Path]) -> list[TrackCase]:
    cases_by_path: dict[Path, TrackCase] = {}

    for root in roots:
        for case in discover_legacy_cases(root):
            cases_by_path[case.path.resolve()] = case
        for case in discover_recursive_cases(root):
            cases_by_path.setdefault(case.path.resolve(), case)

    return sorted(cases_by_path.values(), key=lambda item: (item.hand_group, item.gloss, item.step_label))


def discover_legacy_cases(root: Path) -> list[TrackCase]:
    ...
    for child in sorted(root.iterdir()):
        ...
        if has_tracking_payload(child):
            cases.append(TrackCase(gloss=child.name, step_label="step1000", hand_group=infer_hand_group(child), path=child))
    ...
    for child in sorted(root.iterdir()):
        ...
        if has_tracking_payload(nested):
            cases.append(TrackCase(gloss=gloss, step_label="step100", hand_group=infer_hand_group(nested), path=nested))
        elif has_tracking_payload(child):
            cases.append(TrackCase(gloss=gloss, step_label="step100", hand_group=infer_hand_group(child), path=child))

    return cases


def discover_recursive_cases(root: Path) -> list[TrackCase]:
    if not root.is_dir():
        return []

    cases: list[TrackCase] = []
    for payload in sorted(root.rglob("optim_tracking_ehm.pkl")):
        case_dir = payload.parent
        if not has_tracking_payload(case_dir):
            continue
        step_label = infer_step_label(case_dir)
        ...
        cases.append(
            TrackCase(
                ...
                hand_group=infer_hand_group(case_dir),
                path=case_dir,
            )
        )
...
def infer_step_label(path: Path) -> str | None:
...
def infer_hand_group(path: Path) -> str:
    for part in path.parts:
        if part in {"1_hand", "2_hands"}:
            return part
    return "unknown"
...
def write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    preferred = ["hand_group", "gloss", "step", "frames", "path"]
...
def write_markdown(path: Path, rows: list[dict[str, Any]], pck_thresholds: list[float]) -> None:
    ...
    for row in rows:
        by_case.setdefault((row.get("hand_group", "unknown"), row["gloss"]), []).append(row)
    ...
    for (hand_group, gloss), group in sorted(by_case.items()):
        lines.append(f"## {hand_group} / {gloss}")
        lines.append("")
